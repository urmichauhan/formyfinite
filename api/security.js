...
export async function hashPassword(p) {
  ...
  const result = await scrypt(p, salt, 64);
  return salt + ":" + result.toString("hex");
}
export async function matches(p, stored) {
  const [salt, value] = stored.split(":");
  return timingSafeEqual(Buffer.from(value, "hex"), await scrypt(p, salt, 64));
}
export function vault(hex) {
  if (!/^[a-f0-9]{64}$/i.test(hex || ""))
    throw Error("Set DATA_ENCRYPTION_KEY to 64 hexadecimal characters");
